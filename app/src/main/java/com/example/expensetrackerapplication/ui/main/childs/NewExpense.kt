            ...
            try {
                if(isChecked)
                {
                    if (Global.isCalendarSelected == false)
                    {
                        ...
                        var totAmt = Global.fnFormatFloatTwoDigits(newExpenseViewModel.expenseAmt.value?.toFloat(),logger)
                ...
                {
                    fnShowMessage(getString(R.string.enter_expense),requireContext(),R.drawable.error_bg,logger,LOG_TAG)
                    newExpenseBinding.idENewExpense.isFocusable=true
                    newExpenseBinding.idENewExpense.requestFocus()
                }
            }
